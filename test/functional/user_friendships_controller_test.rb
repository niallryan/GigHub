...
class UserFriendshipsControllerTest < ActionController::TestCase
  ...
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:niall), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:niall), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
      ...
      context "successfully" do
        should "create two user friendship objects" do
          assert_difference 'UserFriendship.count', 2 do
            post :create, user_friendship: { friend_id: users(:maeve).profile_name }
          end
        end
      end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:niall), friend: @friend)
        create(:pending_user_friendship, friend: users(:niall), user: @friend)
        sign_in users(:niall)
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:niall))
        sign_in users(:niall)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:niall))
        create(:accepted_user_friendship, friend: users(:niall), user: @friend)
